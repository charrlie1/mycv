...
def run_v2_v3_pipeline(rgb: np.ndarray) -> None:
    """Execute the v2/v3 image processing pipeline (outputs 01-18)."""
    ...
    save(
        to_u8(mycv.harris_corner_response(Gx.astype(np.float64), Gy.astype(np.float64))),
        "output_13_harris.jpg"
    )
    
    hough = mycv.hough_line_transform(binary, n_thetas=180)
    save(to_u8(hough["accumulator"].astype(np.float64)), "output_14_hough_accum.jpg")
...
def run_motion_tracking(gray: np.ndarray, rng: np.random.Generator) -> None:
    """Run motion detection on synthetic frame pair."""
    H, W = gray.shape[:2]
...
def run_color_tracking(hsv: np.ndarray) -> None:
    """Run HSV color mask and centroid calculation for warm hues."""
...
def run_temporal_smoothing(
    ...
    """Run temporal smoothing with N=5 ring buffer."""
    smoother = mycv.TemporalSmoother(
        n_frames=5, 
        height=height, 
        width=width, 
        channels=3
    ...
    for _ in range(5):
        ...
        noisy_frame = np.clip(
            rgb.astype(np.int16) + frame_noise, 0, 255
        ).astype(np.uint8)
...
def main() -> None:
    """Run the mycv v4 demonstration pipeline."""
